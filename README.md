...
- 动态广场：信息流、点赞、评论、发布（图片上传到 `/uploads`）、图片点击预览
- 我的：资料编辑、头像上传、余额、我的关注、退出登录

**玩伴端（B 端工作台）**

- 申请入驻：城市/区域/介绍/技能标签/相册/服务项目编辑（名称/价格/单位/最低起订），提交即开通
- 工作台：上线/休息一键切换、待接单数/今日订单/累计完成/累计收入统计
- 订单操作：接单/拒绝/开始服务/完成服务，10s 轮询新单
- 资料编辑：与申请表单复用，`/partner/apply` 进入编辑模式
