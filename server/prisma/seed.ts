...
async function main() {
  ...
  // 否则重灌后残留指向已删用户的脏流水）
  for (const t of [
    'Notice', 'Report', 'Announcement', 'PartnerOffDate', 'BalanceLog', 'Message', 'Commission', 'UserCoupon', 'Withdrawal', 'RechargeCard',
  ...
  for (let i = 0; i < PARTNERS.length; i++) {
    ...
    const user = await prisma.user.create({
      data: {
        // 玩伴演示账号 13800001000 ~ 13800001007（与 README/测试引导页一致）
        mobile: String(13800001000 + i),
        nickname: p.name,
        avatar,
        gender: p.gender,
        city: p.city,
